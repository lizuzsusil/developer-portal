...
සේවා කුඩා යෙදුම් SDK එක සත්කාරක කවචය මගින් ක්‍රියාත්මක අවස්ථාවේදී `window.__SEWA_SDK__` හරහා **එන්නත් කරනු ලැබේ**. ඔබ SDK එක `npm install` නොකරයි. ඒ වෙනුවට, ඔබේ කුඩා යෙදුම සංවර්ධනය සඳහා හවුල් වර්ග පැකේජය මත රඳා පවතී, සහ සත්කාරකය ඔබේ යෙදුම කන්ටේනරය තුළ සවි කර ඇති විට සජීවී SDK අවස්ථාව සපයයි.
...
```mermaid
flowchart LR
    A["වර්ග පැකේජය<br/>(devDependency)"] -->|"import type MiniAppSdkInterface"| B["gov-sdk.d.ts<br/>declare global Window.__SEWA_SDK__"]
    B --> C["ඔබේ කේතය<br/>sdk.auth.getUser() — සම්පූර්ණයෙන් ටයිප් කර ඇත"]
    D["සත්කාරක කවචය<br/>(CDN / native bridge)"] -->|"ක්‍රියාත්මක අවස්ථාවේදී එන්නත් කිරීම"| E["window.__SEWA_SDK__"]
    E --> C
```
...
## 1. වර්ග ප්‍රකාශන (`gov-sdk.d.ts`)

සත්කාරකය එන්නත් කරන ගෝලීයයන් TypeScript දැන ගැනීම සඳහා ප්‍රකාශ කරන්න. මෙය [`test-mini-app/src/gov-sdk.d.ts`](/si/docs/getting-started) වෙතින් ගත් ගොනුවයි:

<ImportCodeBlock />

## 2. ඇතුල්වීමේ ස්ථානය (`main.tsx`)

`mount` ශ්‍රිතයක් අපනයනය කරන්න — පරිශීලකයා කුඩා යෙදුම විවෘත කරන විට සත්කාරකය එය DOM කන්ටේනරයක් සහ විකල්ප `initialPath` සමඟ අමතයි. `window.__SEWA_SDK__` ක්‍රියාත්මක අවස්ථාවේදී කියවනු ලැබේ.
