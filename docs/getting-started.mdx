...
You can see the full working example in the `test-mini-app/` reference project. See the [React Mini App How-To](/docs/integration/react-mini-app) for a step-by-step creation guide.
...
```mermaid
flowchart LR
    A["Types package<br/>(devDependency)"] -->|"import type MiniAppSdkInterface"| B["gov-sdk.d.ts<br/>declare global Window.__GSA_SDK__"]
...
No direct SDK import - the provider reads `window.__GSA_SDK__` at runtime. The host sets the initial hash so `HashRouter` opens at the right route; if the host already set the hash (deep link) it is left untouched.
